...
final class StagiaireController extends AbstractController
{
    ...
    #[Route('/test/stagiaire', name: 'test_stagiaire')]
public function testStagiaire(EntityManagerInterface $em): Response
{
    $stagiaire = new Stagiaire();
    $stagiaire->setNom('Test');
    $stagiaire->setPrenom('Stagiaire');
    $stagiaire->setEmail('test@stagiaire.com');

    $em->persist($stagiaire);
    $em->flush();

    return new Response('✅ Stagiaire ID ' . $stagiaire->getId() . ' créé');
}
}
